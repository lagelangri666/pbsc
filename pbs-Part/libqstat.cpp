...
bool job_is_complete(struct attrl *a)
{
  while (a != NULL)
  {
...

{
  ...
  for (p = status; p != NULL; p = p->next)  //p为作业名称的链表
    {
		...
		id_name = p->name;
		std::string s ="----";
		std::string id_state = name+s+state;
		jobid_list.push_back(id_state); 
  	}
  if (DisplayXML == true)
    {
    ...
    if (p_status == NULL)
      ...
      if ((any_failed == PBSE_UNKJOBID) && !*located)
        ...
        *located = TRUE;
        ...
        if (DisplayXML)
        	{
              display_statjob( p_status, print_header, f_opt, user,jobid_list);
        	}
...
vector<string> GetRunningJobs( )  /* I */

{
    ...
    switch (mode)
      ...
      case JOBS:      /* get status of batch jobs */
        any_failed = run_job_mode(have_args, operand, &located, server_out, server_old,
         queue_name_out, server_name_out, job_id_out,jobid_list,errmsg);
        break;
  ...
  vector<string> job = jobid_list;
  vector<string>().swap(jobid_list);
  optind--;
  return job;
  exit(ret_code);
